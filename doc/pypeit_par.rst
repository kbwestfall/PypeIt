...
``spectral_region_mask``  str, list                                       ..                ``atm.toml``, ``hydrogen.toml``  A set of spectral regions to mask during the model (source + telluric) fit to the observed spectrum.  This can be one or more spectral ranges defined directly or strings pointing to TOML files defining the masked regions.  When provided via TOML files, the files can be local or provided by the pypeit package.  See :ref:`telluric_masks` for more information.                      
