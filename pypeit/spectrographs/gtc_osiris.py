...
class GTCOSIRISPlusSpectrograph(spectrograph.Spectrograph):
    ...
    def tweak_standard(self, spec, trim_std_pixs=None, wave_range=None, mask=True):
        ...
        if wave_range is None:
            ...
            if dispname == 'R300R':
                wave_range = (4800.0, 9300.0)
            elif dispname == 'R500R':
                wave_range = (4800.0, 9300.0)
            elif dispname == 'R300B':
                wave_range = (3400.0, 6500.0)
            elif dispname == 'R500B':
                wave_range = (3400.0, 6500.0)
            elif dispname == 'R1000B':
                wave_range = (3500.0, 6300.0)
...
class GTCOSIRISSpectrograph(spectrograph.Spectrograph):
    ...
    # TODO: This is exactly the same as the tweak_standard for the other
    # spectrograph class.  Can we use a common base class for GTC/OSIRIS?
    def tweak_standard(self, spec, trim_std_pixs=None, wave_range=None, mask=True):
        """
        Tweak spectra of a standard star to improve flux-calibration robustness.
