...
def test_spectrum_list_container_basics():
    ...
    assert spec.meta == meta, 'Metadata should be unchanged'
...
def test_spectrum_list_container_io():
    ...
    assert 'METAKEYS' in hdu[0].header and len(hdu[0].header['METAKEYS'].split(',')) == 4, \
        'Bad metadata header information'
    assert 'INT' in hdu[0].header and hdu[0].header['INT'] == meta['int'], \
        'Bad metadata value'

    with pytest.raises(PypeItError):
        # Should fault if given an incorrect number of hdu names
        hdu = spec.to_hdu(hdu_names=['SPECTRUM1', 'SPECTRUM2', 'SPECTRUM3'])

    with pytest.raises(PypeItError):
        # The names must be unique
        hdu = spec.to_hdu(hdu_names=['SPECTRUM', 'SPECTRUM'])
    ...
    assert _spec.meta == spec.meta, 'Metadata dictionary changed'
