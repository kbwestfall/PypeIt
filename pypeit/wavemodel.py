...
def conv2res(wavelength, flux, resolution, central_wl='midpt',
    ...
    if current_resolution is not None:
        if current_resolution < resolution:
            raise PypeItError('Current resolution is lower than desired resolution.')
    ...
    log.info(f"The binning of the wavelength array at {wl_cent:.1f} is: {px_bin:.1f}")
    px_sigma = wl_sigma / px_bin

    log.info(f"Covolving with a Gaussian kernel with sigma = {px_sigma:.1f} pixels")
